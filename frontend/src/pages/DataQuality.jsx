...
import { Database, AlertTriangle, Layers, ArrowUpRight, TrendingUp, Loader2, WifiOff } from 'lucide-react';
...
// ── Helper: normalise importance weights to a 0-100 scale ─────────────────
function normalise(importanceMap) {
...
export default function DataQuality() {
  ...
  const importanceList = normalise(importanceMap);
  ...
  return (
    <div className="w-full text-white animate-in fade-in duration-500">
              ...
              <tbody>
                {missingValuesData.map((row, i) => (
                  <tr key={i} className="border-b border-[#1A1D27] hover:bg-[#1A1D27]/50 transition-colors group">
                    ...
                    <td className={`px-1 py-4 text-right font-semibold flex justify-end items-center gap-2 ${row.status === 'Clean' ? 'text-green-400' :
      ...
      {/* ── LIVE: Top Churn Drivers from ML Model ── */}
      <div className="bg-[#13151D] rounded-[32px] p-8 border border-[#6366F1]/20 mb-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-8 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-200">Top Churn Drivers</h3>
              <p className="text-xs text-gray-500">Live · RandomForest feature importance from ML model</p>
            </div>
          </div>
            ...
            <tbody>
              {dataTypeData.map((row, i) => (
                <tr key={i} className="border-b border-[#1A1D27] hover:bg-[#1A1D27]/50 transition-colors">
                  <td className="px-4 py-4 font-medium text-gray-500">0{i + 1}</td>
                  <td className="px-4 py-4 font-medium text-gray-200">{row.column}</td>
                  <td className="px-4 py-4 text-gray-400">{row.expected}</td>
                  <td className="px-4 py-4 text-gray-300">{row.actual}</td>
                  <td className="px-4 py-4 text-right">
                    {row.match ? (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-500/10 text-green-400 rounded-full text-xs font-semibold uppercase tracking-wider">Match</span>
                    ) : (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-500/10 text-red-400 rounded-full text-xs font-semibold uppercase tracking-wider">Mismatch</span>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#13151D] border border-[#EF4444]/20 rounded-[24px] p-6 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-[#EF4444]/10 text-[#EF4444] border border-[#EF4444]/20 uppercase tracking-wide">⚠ Warning</span>
            </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center py-1.5 border-b border-[#2A2D39]"><span className="text-gray-500">Min</span><span className="text-gray-200 font-semibold">$12</span></div>
              <div className="flex justify-between items-center py-1.5 border-b border-[#2A2D39]"><span className="text-gray-500">Max</span><span className="text-gray-200 font-semibold">$4,840</span></div>
              <div className="flex justify-between items-center py-1.5"><span className="text-gray-500">Outliers</span><span className="text-[#EF4444] font-bold">23 detected</span></div>
            </div>
          </div>
          ...
          <div className="bg-[#13151D] border border-[#F59E0B]/20 rounded-[24px] p-6 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-[#F59E0B]/10 text-[#F59E0B] border border-[#F59E0B]/20 uppercase tracking-wide">⚠ Warning</span>
            </div>
            <p className="text-gray-400 text-xs font-medium uppercase tracking-widest mb-1">Column</p>
            <h4 className="text-xl font-bold text-white mb-4 font-mono">complaint_count</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center py-1.5 border-b border-[#2A2D39]"><span className="text-gray-500">Min</span><span className="text-gray-200 font-semibold">0</span></div>
              <div className="flex justify-between items-center py-1.5 border-b border-[#2A2D39]"><span className="text-gray-500">Max</span><span className="text-gray-200 font-semibold">18</span></div>
              <div className="flex justify-between items-center py-1.5"><span className="text-gray-500">Outliers</span><span className="text-[#F59E0B] font-bold">11 detected</span></div>
            </div>
          </div>
          ...
          <div className="bg-[#13151D] border border-[#EAB308]/20 rounded-[24px] p-6 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-[#EAB308]/10 text-[#EAB308] border border-[#EAB308]/20 uppercase tracking-wide">⚠ Notice</span>
            </div>
            <p className="text-gray-400 text-xs font-medium uppercase tracking-widest mb-1">Column</p>
            <h4 className="text-xl font-bold text-white mb-4 font-mono">tenure_months</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center py-1.5 border-b border-[#2A2D39]"><span className="text-gray-500">Min</span><span className="text-gray-200 font-semibold">1</span></div>
              <div className="flex justify-between items-center py-1.5 border-b border-[#2A2D39]"><span className="text-gray-500">Max</span><span className="text-gray-200 font-semibold">84</span></div>
              <div className="flex justify-between items-center py-1.5"><span className="text-gray-500">Outliers</span><span className="text-[#EAB308] font-bold">7 detected</span></div>
            </div>
          </div>
